<template>
  <div class="lap__cart">
    <card-popup v-if="isCardPopupVisible" @closeCardPopup="closeCardPopup">
    </card-popup>

    <div class="lap__container">
      <h1 class="cart__title">Ваша корзина</h1>
      <p class="lap__description" v-if="!cart_data.length">
        Ваша корзина на данный момент пуста
      </p>
      <p class="lap__description" v-if="cart_data.length">
        Вы выбрали выгодные предложения, оформите заказ пока они есть в наличии
        и не изменилась стоимость
      </p>
    </div>
    <lap-cart-item
    ...
    <div class="lap__bottom">
      ...
      <button class="lap__button-buy" @click="showPopupCard">
        Оформить заказ
      </button>
    </div>
  </div>
